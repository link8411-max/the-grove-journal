...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const post = await getPostBySlug("grove-story", slug);
  if (!post) return {};
...
export default async function GroveStoryArticle({ params }: Props) {
  const { slug } = await params;
  const post = await getPostBySlug("grove-story", slug);
  ...
  return (
    <article className="mx-auto max-w-3xl px-6 pt-20 pb-24">
      ...
      {/* Cross-link Banner */}
      <CrossLinkBanner articleTitle={post.title} category="grove-story" />
    </article>
  );
}
